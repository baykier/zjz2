...
char *opt_WhChams[] = { "[OFF]", "[On]", "[Yellow]", "[Red]", "[Blue]", "[Green]", "[Purple]", "[White]", "[Yellow2]" };
...
void DrawMenu(LPDIRECT3DDEVICE9 pDevice)
{
	...
	if (ShowMenu)
	{
		...
		AddItem(pDevice, " Wallhack", wallhack, opt_WhChams, 8);
		...
		AddItem(pDevice, " Antispectate NUMPAD1/2", antispectate, opt_OnOff, 1); //this is actually usless in menu i just put it to let you know it exist in hack
		AddItem(pDevice, " FASTPARACUTE F1/F2 on/off", FASTPARACUTE, opt_OnOff, 1); //same as antispectate usless in menu
		...
		if (menuselect < 1)
			menuselect = 20;//Current;
	}
}
 
//=====================================================================================================================
//OFFSETS ... NOT IMPORTANT YOU CAN REMOVE THIS ... IT'S JUST FOR ME 
...
DWORD MODULEROS = (DWORD)GetModuleHandleA("zjz.exe");
