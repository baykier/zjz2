...
HRESULT APIENTRY SetTexture_hook(LPDIRECT3DDEVICE9 pDevice, DWORD Sampler, IDirect3DBaseTexture9 *pTexture)
{
	...
	if (walkthrough == 0)
	{
		PacthHook((void *)(MODULEROS + 0x175EF68), (int*)(PBYTE)"\x00\x00\x00\xBF", 4);
	}
	if (walkthrough == 1)
	{
		PacthHook((void *)(MODULEROS + 0x175EF68), (int*)(PBYTE)"\xCD\xCC\xCC\xBD", 4);
	}
 
	//AUTOCLIMB	V3
	if (autoclimb)
		*(BYTE*)(MODULEROS + 0x5445B0) = 0x4C; //on
	else
		*(BYTE*)(MODULEROS + 0x5445B0) = 0x5C; //off
 
	//UBDERWTR	 V3
	if (underwater == 0)
	{
		PacthHook((void *)(MODULEROS + 0x396F8C), (int*)(PBYTE)"\x90", 1);
	}
	if (underwater == 1)
	{
		PacthHook((void *)(MODULEROS + 0x396F8C), (int*)(PBYTE)"\x91", 1);
	}
	//jumphack  V3
	if (jumphack == 1)
	{
		PacthHook((void *)(MODULEROS + 0x543CF2), (int*)(PBYTE)"\xB8\x00\x00\x48\x43\x56\x8D\x71\x6C\x90\x90", 11);
	}
	if (jumphack == 0)
	{
		PacthHook((void *)(MODULEROS + 0x543CF2), (int*)(PBYTE)"\x8B\x41\x50\x56\x8D\x71\x6C\xC6\x41\x7E\x00", 11);
	}
	//GRAVEMODE V3
	if (gravemode)
	memcpy((void*)(MODULEROS + 0x545018), "\x90\x90\x90\x90\x90\x90\x90\x90", 8);
    else
	memcpy((void*)(MODULEROS + 0x545018), "\xF3\x0F\x11\x87\xE0\x00\x00\x00", 8);
 
	//FASTPARACUTE   V3         
    if (GetAsyncKeyState(VK_F1)& 0x8000)        //ON F1 HOTKEY
	{
		*(float*)(*(DWORD*)(*(DWORD*)(*(DWORD*)(*(DWORD*)(*(DWORD*)(MODULEROS + 0x21992EC) + 0x0) + 0x40) + 0x4) + 0x10) + 0x4C) = 990.0f;
	}
	if (GetAsyncKeyState(VK_F2)& 0x8000)       //OFF F2 HOTKEY
	{
		*(float*)(*(DWORD*)(*(DWORD*)(*(DWORD*)(*(DWORD*)(*(DWORD*)(MODULEROS + 0x21992EC) + 0x0) + 0x40) + 0x4) + 0x10) + 0x4C) = 450.0f;
	}
	...
	if (crosshair == 2)
	{
		CrossHair(pDevice, D3DCOLOR_ARGB(255, 255, 0, 0)); //red
	}
	if (crosshair == 3)
	{
		CrossHair(pDevice, D3DCOLOR_ARGB(255, 0, 0, 255)); //blue
	}
	if (crosshair == 4)
	{
		CrossHair(pDevice, D3DCOLOR_ARGB(255, 0, 255, 0)); //green
	}
	if (crosshair == 5)
	{
		CrossHair(pDevice, D3DCOLOR_ARGB(255, 255, 0, 102)); //purple
	}
	...
	{
		*(byte*)(MODULEROS + 0x12D486) = 2; //ON HOTKEY : NUMPAD1
	}
	/* reconnect */
	    if (GetAsyncKeyState(VK_NUMPAD2)& 1)
	{
		*(byte*)(MODULEROS + 0x12D486) = 1; //OFF HOTKEY : NUMPAD2
	}
...
HRESULT APIENTRY Present_hook(IDirect3DDevice9* pDevice, const RECT *pSourceRect, const RECT *pDestRect, HWND hDestWindowOverride, const RGNDATA *pDirtyRegion)
{
	...
	if (ShowMenu)
		//draw background
		DrawBox(pDevice, 71.0f, 86.0f, 200.0f, 310.0f, D3DCOLOR_ARGB(200, 20, 20, 20));//180 = up/down, 200 = left/right
